...
let verify = () => {
  ...
  let code = `${code1.value}${code2.value}${code3.value}${code4.value}${code5.value}${code6.value}`
  console.log(code1.value);

  fetch(`http://localhost:5141/api/Verification/VerifyCode/${code}/${id}`)
    .then(res => res.json())
    .then(function (value) {
...
let resendCode = () => {
  const options = {
    ...
    fetch(`https://localhost:7143/api/VerificationCode/UpdateCode/${id}`, options)
    .then(res => res.json())
