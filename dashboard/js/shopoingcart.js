...
<td class="shoping__cart__item">
    <img src="http://localhost:5141/assets/images/{{PRODUCT-IMAGE}}" alt=""  width="100px" height="100px">
    <h5>{{PRODUCT-NAME}}</h5>
</td>
...
function cartTable() {
  ...
  cartItem.forEach((x) => {
    ...
    if (document.querySelector("#quantity" + x.id) != null) {
      let i = cartItem.filter((x) => x.id == productid);

      let duplicateItems = document.querySelector("#quantity" + x.id);
      var currentValue = parseInt(duplicateItems.value);
      duplicateItems.value = currentValue + 1;
      console.log("yes");
    } else {
      console.log(x);
      let item = CARTTABLETEMPLATE.replace("{{PRODUCT-NAME}}", x.productName)
        .replace("{{PRODUCT-IMAGE}}", x.image)
...
function decreaseQuantity(productId) {
  ...
  let a = carto.indexOf(carto.find((x) => x.id == productId));
  console.log(a);
  carto.splice(a, 1);
  let error = document.querySelector("#error-message" + productId);
...
function increaseQuantity(productId) {
  ...
  let item_to_increase = items.find((x) => x.id == productId);
...
function removeFromCart(id) {
  ...
  let items = JSON.parse(localStorage.getItem("cart"));
  console.log(items);
  var remaining_items = items.filter((x) => x.id != id);
