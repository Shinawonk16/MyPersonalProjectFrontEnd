...
<img src="http://localhost:5141/assets/images/{{CUSTOMER-IMAGE}}"
    alt="sq-sample3" />
<div class="author">
...
let like = (id) => {
    ...
    fetch('http://localhost:5141/api/Like/CreateLike', options)
        .then(res => res.json())
...
function myFunction(id) {
    ...
    fetch(`http://localhost:5141/api/Like/GetLikesByReviewId/${id}`)
    .then(res => res.json())
    .then(function(response){
    ...
    await getAllReviews()
        .then(async function (response) {
            ...
            response.data.forEach(x => {
                fetch(`http://localhost:5141/api/Like/GetLikesByReviewId/${x.id}`)
                    .then(res => res.json())
                    .then(function (likes) {
...
async function getAllReviews() {
    let reviews = await fetch('http://localhost:5141/api/Review/GetAll')
    return reviews.json();
}
async function getLikes(id) {
    let likes = await fetch(`http://localhost:5141/api/Like/GetLikesByReviewId/${id}`)
    return likes.json();
}
...
let Update = () => {
    ...
    fetch('http://localhost:5141/api/Review/UpdateAll', options)
        .then(response => response.json())
        .then(function (res) {
            if (res.status == true) {
