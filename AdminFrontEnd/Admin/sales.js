...
<td style="text-align: center;">
    <div class="email">
        <div class="img" style="text-align: center !important;">
            <img src="http://localhost:5141/assets/images/{{CUSTOMER-IMAGE}}" alt="" style="width: 50px; height: 50px;" class="rounded-circle">
        </div>
...
let getAllSales = () => {
    fetch(`http://localhost:5141/api/Sale/GetAllSalesForTheYearOnEachProduct/${year.value}`)
    .then(res => res.json())
...
let displayDetailsforProduct = (id) => {
    fetch(`http://localhost:5141/api/Sale/GetSalesByProductNameForTheYear/${id}/${year.value}`)
    .then(res => res.json())
    .then(function(response){
...
function findMatches(wordToMatch, sales)
{
    return sales.filter(x => {
        ...
        return x.customerDto.userDto.username.match(regex) || x.orderedDate.match(regex) || x.deleveredDate.match(regex) || x.customerDto.phoneNumber.match(regex);
    })
}
function displayMatches(){
            ...
            const html = matchObj.map(x => {
                ...
                <td style="text-align: center;">
                    <div class="email">
                        <div class="img" style="text-align: center !important;">
                            <img src="http://localhost:5141/assets/images/${x.customerDto.userDto.profilePicture}" alt="" style="width: 50px; height: 50px;" class="rounded-circle">
                        </div>
                        <span style="font-size: 11px;">${x.customerDto.username}</span>
                    </div>
                </td>
