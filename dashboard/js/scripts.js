...
fetch(`http://localhost:5141/api/Payment/Get/${ref}`)
  .then(res => res.json())
  .then(function (response) {
    ...
    let p = PaymentInfo
      ...
      .replace('{{EMAIL}}', response.data.paymentDto.email)
...
let fetchOrderDetails = () => {
  fetch(`http://localhost:5141/api/Order/Get/${orderId}`)
    .then(res => res.json())
    .then(function(response){
      console.log(response)
      cart_info.innerHTML = ""
      total.innerText
      let t = 0
      cartItems.replace('{{Q}}',response.data.quantity)
      response.data.forEach(x => {
        let ct = cartItems
        .replace('{{Q}}',x.quantity)
        .replace('{{PRODUCT-NAME}}',x.orderDto.product.productName)
        .replaceAll('{{TOTAL}}', x.orderDto.amountPaid)
        t += x.orderDto.amountPaid
      })
      total.innerText = "₦" + t
    })
}
