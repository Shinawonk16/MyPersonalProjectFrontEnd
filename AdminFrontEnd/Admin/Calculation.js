...
let displayProfit = () => {
    fetch('http://localhost:5141/api/Sale/CalculateProfit')
        .then(res => res.json())
...
displayProfitForSelectedMonth = () => {
    fetch(`http://localhost:5141/api/Sales/CalculateProfit/${mnth.value}/${year.value}`)
        .then(res => res.json())
        .then(function (response) {
...
let monthlySalseTable = () => {
    fetch(`http://localhost:5141/api/Sales/GetSalesForTheMonth/${sales_month.value}/${year.value}`)
        .then(res => res.json())
        .then(function (response) {
            table_row.innerHTML = "";
            ...
            response.data.forEach(x => {
                ...
                let sales = TABLETEMPLATE
                    .replace('{{COUNT}}', count)
                    .replace('{{PRODUCT}}', x.productDto.productDtoname)
                    .replace('{{IMAGE}}', x.productDto.image)
...
<img src="https://localhost:7143/assets/images/{{CUSTOMER-IMAGE}}" alt="" style="width: 55px; height: 55px;" class="rounded-circle"></div>
...
let getFullInfoOnAProductForTheMonth = (id) => {
    fetch(`http://localhost:5141/api/Sales/GetSalesByProductNameForTheMonth/${id}/${sales_month.value}/${year.value}`)
        .then(res => res.json())
        .then(function (response) {
            ...
            response.data.forEach(x => {
                let sales = SALESTABLETEMPLATE
                    ...
                    .replace('{{CUSTOMER-NAME}}', x.customerDto.users.userName)
                    .replace('{{PHONE}}', x.customerDto.users.phoneNumber)
                count++;
                table_row.innerHTML += sales;
            })
        })
...
let calculateNetProfit = () => {
    ...
    swal({
        ...
        if (extraexp.value == "" || monthNet.value == "") {
            ...
            fetch(`http://localhost:5141/api/Sales/CalculateNetProfit/${m}/${y}/${extraexp.value}`)
                .then(res => res.json())
                .then(function (response) {
