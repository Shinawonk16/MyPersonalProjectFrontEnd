...
let IMAAGETEMPLATE = `<div class="d-flex flex-column align-items-center text-center p-3 py-5"><img class="rounded-circle mt-5" width="150px" src="http://localhost:5141/assets/images/{{PROFILE-IMAGE}}">
...
let viewProfile = () => {
    ...
    fetch(`http://localhost:5141/api/Manager/Get/${id}`)
    .then(response => response.json())
    .then(function(response){
        profile_image.innerHTML = "";
            user_name.value = response.data.userDto.userName;
            fullname.value = response.data.userDto.userName;
            email.value = response.data.userDto.email;
            phone.value = response.data.userDto.phoneNumber;
            let image = IMAAGETEMPLATE
            .replace('{{PROFILE-IMAGE}}',response.data.userDto.profilePicture)
            .replace('{{NAME}}',response.data.userDto.userName)
            .replace('{{EMAIL}}',response.data.userDto.email)
