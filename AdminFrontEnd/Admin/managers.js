var admin = document.querySelector(".manager");
...
<div class="card">
    ...
    <div class="card-body text-center">
        <img src="http://localhost:5141/assets/images/{{IMAGE}}"
...
function fetchRoles() {
  fetch("http://localhost:5141/api/Role/GetAllRole")
    .then((res) => res.json())
...
let displayManagers = () => {
  ...
  console.log(rol);
  admin.innerHTML = "";
  fetch("http://localhost:5141/api/Manager/GetAllAdmins")
    .then((res) => res.json())
    .then(function (response) {
      admin.innerHTML == "";
      response.data.forEach((x) => {
        let manager = MANAGERSTEMPLATE.replace("{{IMAGE}}", x.userDto.profilePicture)
          .replace("{{USER-NAME}}", x.userDto.userName)
          .replace("{{ROLE}}", x.userDto.role)
          .replace("{{DESCRIPTION}}", x.userDto.roleDescription)
          .replace("{{USER-ID}}", x.userDto.id);
...
let displayFormaddRole = () => {
  ...
  form.innerHTML = `<div class="">
    <form action="" id="add-category">
        <div class="form_wrap">
            <div class="form_item">
                <input type="text" name="roleName" id="roleName" required class="form-control" placeholder="Role Name">
            </div>
        </div>
        </br>
        <div class="form_wrap">
            <div class="form_item">
                <input type="text" name="roleDescription" id="roleDescription" required class="form-control" placeholder="Role Description">
  ...
  swal({
    ...
    if (isConfirm) {
      ...
      fetch(`http://localhost:5141/api/Role/AddRole`, options)
        .then((res) => {
          console.log(res);
          return res.json();
        })
  ...
  if (loginId == id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`http://localhost:5141/api/Manager/Delete/${id}`, options)
          .then((res) => res.json())
          .then(function (response) {
            if (response.success == true) {
              Swal.fire(
                "Manqger Deleted",
                "Manager has been deleted and can no longer have access to the system",
                "success"
              );

              location.href =
                "http://127.0.0.1:5500/themebootstrap/html/login.html";
            } else {
